...
struct SmallBoard {
    ...
    SmallBoard() {                                                      //The constructor, runs automatically the moment the SmallBoard is created
        ...
        isWon = false;                                                  // to check if this small board is finished without rechecking all 9 cells every time
        winner = Player::NONE;
    }
};
...
struct GameState {
    ...
    bool isBoardForced;         // returns true if next player must play in forced board
...
// One move — where on the board a player plays
struct Move {
    ...
    // Normal constructor — creates a move with real coordinates
    ...
    // Is this move valid (was it actually set)?
    bool isValid() const {
        return bigRow != -1;
    }

    // Convert YOUR move → teacher's GameMove (flat 0-8 coordinates)
    ...
    // Convert teacher's GameMove → YOUR Move                                    //Conversion from (0-8,0-8) coordinates from the getMove()
    static Move fromGameMove(const GameMove& gm) {                               //static says that the function belongs to the type not to the instance
