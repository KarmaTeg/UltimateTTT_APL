...
public:
    ...
    static bool checkSmallWin(const SmallBoard& board, Player player) {                                         //static says that no instance is needed, it can be calles as GameRules::checkSmallWin(...)
    // Check 3 rows
    for (int r = 0; r < 3; r++)
    ...
    // 2. Check if that small board is now won
    ...
    // 3. Calculate the next forced board
    ...
    if (isSmallBoardPlayable(state.boards[nextRow][nextCol])) {
        ...
        // That board is already won or full — free choice
    ...
    static std::vector<Move> getLegalMoves(const GameState& state) {                                              //Initialization
        ...
        if (state.isBoardForced) {
            // if forced — only look in that one small board
            ...
            // if free — look in all playable small boards
